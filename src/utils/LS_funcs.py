...
def find_complex_words(tokens: list, HSK_thresh:int = 5, freq_thresh:float = 0.97):
    ...
    complex_freq = [token for token in tokens if (token in blcu.index and blcu.loc[tokens[0]].values[0]<freq_thresh)] # find all tokens below freq level
